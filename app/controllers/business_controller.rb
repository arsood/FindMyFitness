class BusinessController < ApplicationController
	...
	def search
		...
		if search_location && search_location != ""
			p "Geo worked"
			@businesses = Business.where("LOWER(name) LIKE ? OR LOWER(description) LIKE ?", search_query, search_query).within(20, :origin => search_location).by_distance(:origin => search_location).includes(:business_services).paginate(:page => params[:page], :per_page => 10)
		else
			begin
				...
				if params[:category]
					@businesses = Business.where(business_type: params[:category]).within(5, :origin => [@user_loc["latitude"], @user_loc["longitude"]]).by_distance(:origin => [@user_loc["latitude"], @user_loc["longitude"]]).includes(:business_services).paginate(:page => params[:page], :per_page => 10)
				else
					@businesses = Business.where("LOWER(name) LIKE ? OR LOWER(description) LIKE ?", search_query, search_query).within(20, :origin => [@user_loc["latitude"], @user_loc["longitude"]]).by_distance(:origin => [@user_loc["latitude"], @user_loc["longitude"]]).includes(:business_services).paginate(:page => params[:page], :per_page => 10)
				end
			rescue
				@fail = true
				@businesses = []
	...
	def get_photos
		...
		@bus_photos = BusinessPhoto.where(business_hash: @bus_hash).where(contributor_id: session[:user_id]).paginate(:page => params[:page], :per_page => 8).order(created_at: :desc)
